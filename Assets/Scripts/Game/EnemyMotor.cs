...
namespace DaggerfallWorkshop.Game
{
    ...
    public class EnemyMotor : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // Add doors to ignore for pathing
        ...
        void Move()
        {
            ...
            // If we are pathing to the target, use the next path point
        ...
        void RayCheckForObstacle(Vector3 direction)
        {
            ...
            RaycastHit hit;
            Vector3 p1 = transform.position + controller.center + (Vector3.up * -controller.height * 0.15f);
            Vector3 p2 = p1 + (Vector3.up * controller.height * .30f);
            ...
            if (Physics.CapsuleCast(p1, p2, controller.radius, direction, out hit, checkDistance))
            {
                ...
                else
                {
                    ...
                    if (!secondRayHit || (lowHit.distance < highHit.distance - 0.3f))
                    {
                        obstacleDetected = false;
        ...
        void CreatePath()
        {
            ...
            // Remember which points we have used as ray origins or destinations to eliminate searching same points multiple times
            ...
            foreach (Vector3 origin in usedOrigins)
            {
                ...
                if (!resultCloser || (!success && !sawDestination))
